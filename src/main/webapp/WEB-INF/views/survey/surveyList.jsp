...
<%--  			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>			
 --%> 			
 				<input type="hidden" name="sessionID" value="${sessionScope.member_idx}"/>
				<sec:csrfInput />
									...
									<c:choose>
 										<c:when test="${one.survey_choices eq '' || one.survey_choices eq null}">
											<div class="input-group mb-3">
												<input type="text" name="answer${status.count}" id="answer${status.count}" class="form-control" placeholder="${one.survey_question}" aria-label="${one.survey_question}" aria-describedby="basic-addon1">
											</div>
										</c:when>
